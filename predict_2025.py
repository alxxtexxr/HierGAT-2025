...
def predict_all(args):
    ...
    if cross_validate:
        ...
        if dataset_name == 'mphoi':
            ...
            t_all = []
            for subject_id in test_subject_ids:
                out = outputs_per_subject[subject_id][0]
                tar = outputs_per_subject[subject_id][1]
                t0 = tar['sub-activity_recognition'][:, :, 0].flatten()
                o0 = out['sub-activity_recognition'][:, :, 0].flatten()
                o0 = o0[t0 != -1]
                t0 = t0[t0 != -1]
                t1 = tar['sub-activity_recognition'][:, :, 1].flatten()
                o1 = out['sub-activity_recognition'][:, :, 1].flatten()
                o1 = o1[t1 != -1]
                t1 = t1[t1 != -1]
                t = np.hstack((t0, t1))
                o = np.hstack((o0, o1))
                t_all = np.hstack((t_all, t))
                o_all = np.hstack((o_all, o))
            confusion_matrix = sklearn.metrics.confusion_matrix(t_all, o_all, labels=range(len(subactivity_names)))
            plot_confusion_matrix(confusion_matrix, subactivity_names, normalize=True, title='',
                                  filename=os.path.join(save_visualisations_dir,
                                                        'confusion_subactivity_recognition_mphoi.pdf'))
        if dataset_name == 'cad120':
            ht_all = []
            ho_all = []
            ot_all = []
            ...
            for subject_id in test_subject_ids:
                out = outputs_per_subject[subject_id][0]
                tar = outputs_per_subject[subject_id][1]
                ht0 = tar['sub-activity_recognition'][:, :, 0].flatten()
                ho0 = out['sub-activity_recognition'][:, :, 0].flatten()
                ho0 = ho0[ht0 != -1]
                ht0 = ht0[ht0 != -1]
                ot0 = tar['affordance_recognition'][:, :, 0].flatten()
                oo0 = out['affordance_recognition'][:, :, 0].flatten()
                oo0 = oo0[ot0 != -1]
                ot0 = ot0[ot0 != -1]
                ot1 = tar['affordance_recognition'][:, :, 1].flatten()
                oo1 = out['affordance_recognition'][:, :, 1].flatten()
                oo1 = oo1[ot1 != -1]
                ot1 = ot1[ot1 != -1]
                ot2 = tar['affordance_recognition'][:, :, 2].flatten()
                oo2 = out['affordance_recognition'][:, :, 2].flatten()
                oo2 = oo2[ot2 != -1]
                ot2 = ot2[ot2 != -1]
                ot3 = tar['affordance_recognition'][:, :, 3].flatten()
                oo3 = out['affordance_recognition'][:, :, 3].flatten()
                oo3 = oo3[ot3 != -1]
                ot3 = ot3[ot3 != -1]
                if subject_id == 'Subject4' or 'Subject5':
                    ot = np.hstack((ot0, ot1, ot2, ot3))
                    oo = np.hstack((oo0, oo1, oo2, oo3))
                else:
                    ot4 = tar['affordance_recognition'][:, :, 4].flatten()
                    oo4 = out['affordance_recognition'][:, :, 4].flatten()
                    oo4 = oo4[ot4 != -1]
                    ot4 = ot4[ot4 != -1]
                    ot = np.hstack((ot0, ot1, ot2, ot3,ot4))
                    oo = np.hstack((oo0, oo1, oo2, oo3,oo4))
                ht_all = np.hstack((ht_all, ht0))
                ho_all = np.hstack((ho_all, ho0))
                ot_all = np.hstack((ot_all, ot))
                oo_all = np.hstack((oo_all, oo))
            confusion_matrix = sklearn.metrics.confusion_matrix(ht_all, ho_all, labels=range(len(subactivity_names)))
            plot_confusion_matrix(confusion_matrix, subactivity_names, normalize=True, title='',
                                  filename=os.path.join(save_visualisations_dir,
                                                        'confusion_subactivity_recognition_cad120.pdf'))
            confusion_matrix = sklearn.metrics.confusion_matrix(ot_all, oo_all, labels=range(len(affordance_names)))
            plot_confusion_matrix(confusion_matrix, affordance_names, normalize=True, title='',
                                  filename=os.path.join(save_visualisations_dir,
                                                        'confusion_affordance_recognition_cad120.pdf'))
        if dataset_name == 'bimanual':
            o_all = []
            t_all = []
            for subject_id in test_subject_ids:
        ...
        if save_visualisations_dir is not None and os.path.isdir(save_visualisations_dir):
            ...
            pdb.set_trace()
            t0 = targets['sub-activity_recognition'][:, :, 0].flatten()
            o0 = outputs['sub-activity_recognition'][:, :, 0].flatten()
            o0 = o0[t0 != -1]
            t0 = t0[t0 != -1]
            # t1 = targets['sub-activity_recognition'][:,:,1].flatten()
            # o1 = outputs['sub-activity_recognition'][:,:,1].flatten()
            # o1 = o1[t1!=-1]
            # t1 = t1[t1!=-1]
            # t = np.hstack((t0,t1))
            # o = np.hstack((o0,o1))
            confusion_matrix = sklearn.metrics.confusion_matrix(t0, o0, labels=range(num_subactivities))
            plot_confusion_matrix(confusion_matrix, subactivity_names, normalize=True, title='',
                                  filename=os.path.join(save_visualisations_subject_dir,
                                                        'confusion_subactivity_recognition.pdf'))

            # confusion_matrix = sklearn.metrics.confusion_matrix(targets['sub-activity_prediction'], outputs['sub-activity_prediction'],
            #                                                     labels=range(num_affordances))
            # plot_confusion_matrix(confusion_matrix, affordance_id_to_name, normalize=True, title='',
            #                             filename=os.path.join(save_visualisations_subject_dir, 'confusion_subactivity_prediction.pdf'))
...
def plot_confusion_matrix(cm, classes, filename=None, normalize=False, title='Confusion matrix', cmap=plt.cm.Blues):
    ...
    # pdb.set_trace()
    if normalize:
        cm = cm.astype('float') / (cm.sum(axis=1)[:, np.newaxis] + 1e-30)
    ...
    for i, j in itertools.product(range(cm.shape[0]), range(cm.shape[1])):
        if cm[i, j] >= 0.005:
            plt.text(j, i, '{:.2f}'.format(cm[i, j])[1:], fontsize=7, verticalalignment='center', horizontalalignment="center",
