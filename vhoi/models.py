...
class TGGCN(nn.Module):
    ...
    def forward(self, x_human, x_objects, objects_mask, human_segmentation=None, objects_segmentation=None,
        ...
        # pdb.set_trace()
        x = cat_valid_tensors([x_human, x_objects, x_geometry], dim=2)
        # temporal information
        temporal_x = x.reshape(x.shape[1], -1, x.shape[3])
        temporal_x = self.temporal_self_attn(temporal_x)
        temporal_x = temporal_x.view(batch_size, -1, num_humans+num_objects+1, x.shape[3])
        ...
        spatial_x = spatial_x.view(batch_size, -1, num_humans + num_objects + 1, x.shape[3])
